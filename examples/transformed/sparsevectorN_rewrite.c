...
int sparsevectorN(float epsilon, int size, float q[], float T, int NN, float __SHADOWDP_ALIGNED_DISTANCE_q[], float __SHADOWDP_SHADOW_DISTANCE_q[])
{
  ...
  while (count < NN && (i < size))
  {
    ...
    __SHADOWDP_v_epsilon = ((q[i] + eta_2) >= T_bar) ? (__SHADOWDP_v_epsilon + (0.5 / NN) * epsilon) : (__SHADOWDP_v_epsilon);
    if ((q[i] + eta_2) >= T_bar)
    {
